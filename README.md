...
* Print() : est capable de fonctionner avec pratiquement tous les types de données offerts par Python. Chaînes, nombres, caractères, valeurs logiques, objets
  ...
  * La manière dont nous passons les arguments à la print()fonction est la plus courante en Python et s'appelle la manière positionnelle (ce nom vient du fait que la signification de l'argument est dictée par sa position, par exemple, le deuxième argument sera affiché après le premier, et non l'inverse)
